...
public class DashAbility : MonoBehaviour
{
    ...
    IEnumerator DashRoutine(float dir)
    {
        dashing = true; cooling = true;

        Vector2 saved = motor.Velocity;
        float t = 0f;
        while (t < cfg.dashDuration)
        {
            motor.SetVelocity(new Vector2(cfg.dashSpeed * dir, 0f));
        ...
        motor.SetVelocity(new Vector2(saved.x, motor.Velocity.y));
        dashing = false;
